...
> **本流程已固化为 skill**：日常直接用 `/oryxos-lesson-dev <节号>`（`.agents/skills/oryxos-lesson-dev/`），它按本指导 + 《Harness 设计》的门禁自动走完全程、在 tasks 后停等确认。本文档保留为原理讲解和手动兜底。
...
```text
/speckit-plan 技术栈：Go 1.26+ + Eino core + Eino-ext；DeepSeek 使用原生 connector 并沿用其官方默认地址，MiniMax 使用 OpenAI connector 且在工厂内固定官方兼容地址。动手前用 go.mod、模块缓存和 go doc 核实锁定版本的两个 connector API。实例级启动配置只声明 provider 名和环境变量凭证；Profile 只选择 provider/model/temperature；明确 Provider 的 connector、协议适配和端点策略由各自工厂封装，不接受用户配置 base URL。工厂按 provider.name 注册，模型实例按 Profile.name 隔离。OryxOS `llm.ChatModel` 同时提供 `Generate` 和 `Stream`；Provider 适配层把 Eino Stream 转换为 OryxOS 增量事件，并在流结束时合并出唯一完整 `llm.Response`。一次逻辑流只落一条 `llm_calls`：正常完成记成功，启动失败、中途读失败或未完成就关闭记失败。本节不把 Stream 接入 ReAct、CLI 或 Web/SSE。SQLite + GORM 使用 glebarez/sqlite + modernc.org/sqlite，保持纯 Go 和 CGO_ENABLED=0；建表使用手工 SQL migration，不依赖 GORM AutoMigrate。模块落位：Profile 相关归 internal/profile，ProviderService 和 connector 工厂归 internal/provider，LlmCall GORM Model、Repository 和 migration 归 internal/store。测试按课件 harness 执行：profile_loader_test.go/provider_service_test.go/eino_adapter_test.go/tool_schema_adapter_test.go/llm_call_repository_test.go，覆盖 DeepSeek/MiniMax 工厂路由、同厂商 Profile 隔离、坏 Profile 隔离、Generate/Stream 成败审计落账、Stream 完整响应合并和不自动执行 Tool；provider_smoke_test.go 使用 //go:build integration，默认 CI 跳过。完成门禁为 go test ./...、go vet ./...、CGO_ENABLED=0 go build ./cmd/oryxos 全绿。
```
...
- FR3：上层传入 sessionID、Profile、OryxOS `[]llm.Message`，系统按 Profile 选中对应模型。同步路径返回 OryxOS `llm.Response`；流式路径先返回 OryxOS 增量事件，正常结束时再返回一个完整 `llm.Response`，然后返回 `io.EOF`。
...
- FR5：每次逻辑调用不论成败都只落一条审计（`llm_calls` 表）：provider、model、token 用量、耗时、success 标识、失败原因，按 session 关联。Stream 只在收到完整终态响应时记成功；启动失败、中途读失败和提前关闭均记失败。
- FR6：凭证只从环境变量读取，代码、配置文件、日志里都不出现明文。

**明确不做（边界）。** ReAct 循环本身、工具的真正执行、fallback/熔断/hedge racing、成本聚合看板，以及 ReAct/CLI/Web 的流式链路和 SSE/WebSocket——全部属于后续特性。本节仅交付 Provider 窄端口的 Stream 能力与兼容性回归。

**验收标准。** 以 `docs/require/第16节` 的唯一匹配课件为准：可自动化部分由课件 harness 承载。硬门槛包括 DeepSeek/MiniMax 工厂路由不串台、同一 Provider 的两个 Profile 实例隔离、引用错误名有清晰报错、坏 Profile 不阻断合法 Profile、Generate/Stream 每次逻辑调用恰好一条审计、Stream 增量顺序与完整响应合并正确、失败审计包含 `success=false` 与脱敏 `error_message`、带工具请求不自动执行。人工项包括两个 Eino-ext connector 的本地 API/依赖确认、Generate/Stream 真实冒烟和无明文 key。

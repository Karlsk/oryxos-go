...
│ kimi-search  go_tool  Kimi 搜索      45  2.2%    ● 启用    │
└─────────────────────────────────────────────────────────────┘
```

**交互**：
- 行点击 → 右侧抽屉显示 Tool Schema（参数定义）和最近 20 次调用记录
- 「来源」徽章颜色：builtin 蓝 / mcp 紫 / go_tool 绿（对应 [CLAUDE.md §9.7 tool_invocations.source](../../CLAUDE.md)）
...
> **不引入 SSE / WebSocket**：核心阶段同步阻塞 + 轮询足够；M3 评估 SSE（按 [plan.md §Performance Goals](../../specs/008-agent-web-service/plan.md) goroutine 支撑 100+ 并发）。
...
- 部署在企业内部，前置 API gateway（Nginx / Kong）做 SSO 透传

...
OryxOS 是基于 Go 1.26 实现的面向企业场景的 **Agent OS**。它装在企业自己的 K8s 或服务器上，作为统一底座，在底座上跑各种业务 Agent（运维助手、客服助手、HR 助手、销售助手、知识管理助手等），共享一套渠道接入、模型路由、工具调用、记忆系统、沙箱执行能力。数据完全留在企业自己的基础设施，不锁任何云生态。

业界已经有开源 Agent 项目把这套设计验证过（OpenClaw 用 Node.js，Hermes Agent 用 Python），但 Go 生态没有任何项目把"Agent OS"作为定位。Go 是云原生与基础设施的事实语言，Eino 已经提供底层模型能力抽象，Eino-ext 提供主流模型厂商连接器，缺的就是包含自主 ReAct 运行时和企业治理能力的上层"Agent OS"。OryxOS 填这个位置。
...
1. **核心阶段**：先把 Agent OS 的运行时内核用 Go 做扎实，这一层在能力上对齐业界开源 Agent OS 的基础层
...
OryxOS 通过 Provider 抽象层对接主流大模型（DeepSeek、通义、Kimi、智谱、混元、豆包、MiniMax、Anthropic、OpenAI 等），Agent 不感知具体调的是哪家模型，运行时切换无 lock-in。文心、Gemini、Ollama 等其他 Provider 放在扩展阶段接入。
...
| **AgentService** | CLI、Web Service、定时任务三种触发源共用的 Agent 调用入口，负责调用 ReAct 循环并持久化 Session；同一个 Agent 不因触发源不同而走不同业务链路 |
| **AgentScheduler** | 定时任务触发模块，按 Profile 的 cron 配置到点生成消息，并通过 AgentService 调用 Agent；它是第三种触发源，不是独立的 Agent 执行引擎 |
...
| **易接入** | 基于标准 Go / Gin 工程结构，单二进制部署，既能跟企业现有的 ERP、CRM、CMDB、SSO 等业务系统对接，也能直接融入 K8s、可观测、监控等云原生基础设施，运维工具链复用现有 Go 生态 |
| **可观测** | 标准的 Prometheus 指标（`client_golang`）、结构化 JSON 日志、健康检查接口、Web 仪表板，适配企业现有监控告警体系 |
...
├── memory/
│   └── MEMORY.md      # 长期记忆文件
├── mcp_servers.yaml   # MCP Server 连接配置
...
- `mcp_servers.yaml` 集中定义 MCP Server 的连接信息；Profile 的 `mcp_servers` 字段只按名称引用其中的配置
...
```yaml
name: string                    # Profile 唯一运行时标识，供 CLI、API、Session、Scheduler 使用
description: string             # 描述

identity:
  agent_name: string            # Agent 展示名称（可重复，不作为运行时查找键）
...
notify_channels:
  - name: string                # 通知渠道名称，供 notify Tool 选择
    type: webhook               # 核心阶段支持通用 Webhook
    url: ${NOTIFY_WEBHOOK_URL}  # Webhook 地址，通过环境变量注入

schedules:
  - id: string                  # 定时任务唯一标识
    cron: string                # cron 表达式
    timezone: Asia/Shanghai     # 时区
    message: string             # 到点后发给 Agent 的消息
    enabled: true               # 是否启用
...
接到用户消息
  └─ 追加到 Session 对话历史
     └─ 组装 Prompt（system prompt + Bootstrap + 长期 Memory + 对话历史 + 可用 Tool 列表）
...
**实现要点：**
- 核心循环约数十行 Go 代码，自己实现而不依赖 Eino 的 ADK Agent 抽象（`adk.NewChatModelAgent`，它内部已封装 ReAct，但 OryxOS 选择自己实现以保持对循环逻辑的完全控制）
- 最大迭代次数可在 Profile 里覆盖
- 每次 LLM 调用和 Tool 调用都记录结构化日志，并分别写入 `llm_calls`、`tool_invocations`
...
| `notify` | 通知 | 把消息发送到 Profile 配置的通知渠道；核心阶段通过通用 Webhook 推送 |

`notify` 与用于普通 API 请求的 `http_post` 分工不同：它表达“主动发送通知”的业务语义，根据 Profile 的 `notify_channels` 选择目标。核心阶段的 Webhook 地址同样受 Sandbox 域名白名单约束，每次发送都写入 `tool_invocations`；飞书、企业微信、钉钉等专用通知适配器放在扩展阶段。

核心阶段的调用形式为 `notify(content, channel?)`。Profile 只配置一个通知渠道时，省略 `channel` 自动使用该渠道；配置多个渠道时必须传入 `channel`，按该 Profile 内唯一的 `notify_channels[].name` 精确匹配。核心阶段不默认广播，避免重复或误推送。

Tool 失败后仅在执行结果明确标记为可重试，且调用具备幂等性或幂等键时自动重试，默认指数退避、最多三次。`write_file`、`shell`、`http_post`、`notify`、`save_memory` 等可能产生外部副作用的非幂等调用默认不自动重试，避免重复写入、重复执行或重复推送。
...
`.oryxos/mcp_servers.yaml` 是 MCP Server 连接配置的唯一来源，每项至少包含唯一 `name` 和 `transport`。stdio 类型配置 `command`、可选 `args` 和 `env`；远程类型配置 `url` 和必要的认证环境变量。Profile 的 `mcp_servers` 只引用这里定义的名称，不重复保存连接细节或明文凭证。
...
> 注：Go 没有 JVM SecurityManager 这类语言级安全管理器，核心阶段只能靠应用层白名单校验。完整的 Docker/K8s 容器级沙箱放在扩展功能。
...
Web Service 是 OryxOS 的对外完整门面，业务系统通过 REST API 接入 OryxOS 的所有能力。这是 OryxOS 区别于偏个人定位的 OpenClaw、Hermes 的关键能力。核心阶段基于 Gin 实现 HTTP 服务。
...
| 模式 | 方式 | 适用 |
|------|------|------|
| 同步调用 | `POST /api/v1/agents/{name}/invoke` 等返回 | Stateless 短任务 |
...
Session 是用户和 Agent 一次对话的上下文容器，包含起止时间、用户身份、Profile 标识、对话历史、当前上下文、临时变量。Session 标识由 Channel、用户、Profile `name` 联合生成；`identity.agent_name` 只是展示名称，不参与 Session 标识计算。

核心阶段 Session 数据持久化到本地 SQLite（`.oryxos/sessions/` 下）。重启 OryxOS 后，正在进行的 Session 可以恢复。组装上下文时先按 Profile 的 `max_history_turns` 保留最近 N 轮；如果仍超过 LLM 的 context window，再继续截断早期对话并保留近期对话。

定时触发没有自然用户时，使用系统调度渠道和系统身份生成 Session，其中系统身份按 `schedule.id` 区分，保证同一任务的历次触发可以连续积累上下文、不同任务互不串话。

---

### 5.10 定时任务（第三种触发源）

定时任务让 Agent 在没有人工请求时也能按计划运行。CLI 和 Web Service 是“人推”，`AgentScheduler` 是“钟推”；三种触发源最终都调用同一个 `AgentService`，复用相同的 ReAct、Tool、Memory、Session 和审计链路。

核心阶段要求：

- OryxOS 启动常驻服务时扫描所有 Profile 的 `schedules`，按 cron 表达式和时区注册启用的任务
- 到点后 `AgentScheduler` 使用配置的 `message` 生成一次 Agent 输入，通过 `AgentService` 触发完整 ReAct 循环
- 同一个定时任务上一次尚未结束时，下一次触发直接跳过，不排队也不并行执行两份
- 单次任务失败只记录错误，不得导致调度器退出或影响后续触发；成功和失败都沿用 Session、`llm_calls`、`tool_invocations` 留痕
- 每个定时任务使用独立且稳定的系统 Session：同一 `schedule.id` 的历次触发复用 Session、不同 `schedule.id` 相互隔离；生成的 `session_id` 写入结构化日志，可供 Session API 查询
- 定时定义以 Profile YAML 为事实来源，重启后重新加载；核心阶段不提供任务状态查询、执行历史、启停和立即执行等管理接口

任务状态持久化、独立执行历史和管理接口放在扩展阶段，因此核心阶段不新增 `scheduled_tasks`、`task_executions` 表。
...
| 启动和状态 | `oryxos gateway` | 启动常驻 Agent Runtime 和定时任务；多 Channel 承载放在扩展阶段 |
...
- **专用通知适配器**：在核心阶段通用 Webhook 的基础上，增加企业微信、飞书、钉钉、Slack 等出站通知适配器
...
- **定时任务管理**：用 `scheduled_tasks`、`task_executions` 持久化运行状态和独立执行历史，提供任务查询、历史查询、立即执行、启用和停用接口
- **集群化部署与高可用**：多节点协同通过 Nacos Go SDK 或 etcd 完成，节点故障自动迁移负载
...
- **剩余项目文档**：API 参考文档、生产部署与完整运维手册、贡献者指南 CONTRIBUTING.md、典型场景使用手册
- **Skills Marketplace**：社区贡献的 Skill 共享平台，兼容 agentskills.io 开放标准，跟 OpenClaw 和 Hermes Agent 兼容
- **SDK 多语言支持**：OryxOS 原生用 Go 实现，客户端 SDK 优先级 Java → Python → TypeScript → Go
- **可视化 Profile 编辑器**：让非工程师也能配置 Agent，接近 Dify 的 Agent 配置界面
- **Native 文件生成**：不依赖 LibreOffice 直接生成 pptx、docx、xlsx（Go 用 excelize / unioffice 等）
...
- **RISC-V 和边缘部署**：跑在 Raspberry Pi、边缘网关（Go 原生交叉编译 `GOARCH=riscv64`，单二进制）
...
- Tool 调用仅在明确可重试且具备幂等性或幂等键时自动重试，默认指数退避最多三次；非幂等 Tool 默认不自动重试
...
- **Go**：1.26 及以上
- **操作系统**：Linux 主流发行版（Ubuntu 22.04+、CentOS 8+、Debian 11+、Alibaba Cloud Linux 3、Rocky Linux），通过 `CGO_ENABLED=0` 可交叉编译到各主流平台
- **存储驱动**：SQLite 使用纯 Go 的 `modernc.org/sqlite`，不依赖 CGO，保证单二进制和跨平台交叉编译能力
...
用户编辑配置 Agent 人格、Provider、Tool 列表、Channel、通知渠道和定时规则
用户执行 oryxos chat --profile <name>
  → OryxOS 加载 Profile
  → 初始化 Provider 连接
  → 注册 Tool 到 Agent 工具池
  → 把 Bootstrap 文件加载到系统提示词
  → 加载 MEMORY.md 作为长期上下文
  → Agent 进入待对话状态
```
...
```
消息从触发源进来（CLI 输入、HTTP API 调用，或 AgentScheduler 到点生成消息）
  → 接入层转换成内部统一格式
  → AgentService 作为统一入口接收消息
  → Agent 查询 Session 上下文
  → 组装 LLM Prompt（Bootstrap + 长期 Memory + 对话历史 + 可用 Tool 列表）
  → 调用 LLM Provider 获取响应，每次调用写入 llm_calls
  → [有 Tool 调用] → OryxOS 执行 Tool 并写入 tool_invocations → 结果回传给 LLM 继续生成
  → 把用户消息、Tool 消息和最终响应持久化到 Session
  → CLI/HTTP 触发时把最终响应返回调用方；定时触发需要外发时由 Agent 调用内置 notify 或推送类 MCP Tool
  → 所有动作落结构化日志
```
...
```
用户第一次跟 Agent 说话
  → OryxOS 用 Channel+用户+Profile name 联合 ID 查活跃 Session
  → [无活跃 Session] → 创建新 Session，初始化空对话历史
  → [有活跃 Session] → 恢复 Session 上下文
后续消息追加到 Session 对话历史
  → 先按 max_history_turns 保留最近 N 轮
  → [仍超过模型上下文] → 继续截断早期对话，保留近期（扩展阶段做总结压缩）
...
```
oryxos serve 或 oryxos gateway 启动
  → AgentScheduler 扫描 Profile 的 schedules 并注册启用任务
cron 到点
  → 检查同一任务是否正在执行
  → [正在执行] 跳过本次触发并记录日志
  → [未执行] 用 schedule.message 生成输入
  → 调用 AgentService，复用流程三的完整消息处理链路
  → 执行结果写入 Session、llm_calls、tool_invocations
```
...
核心阶段的定时任务定义随 Profile YAML 持久化，不单独建立任务状态表；每次钟推执行沿用 Session、Tool Invocation 和 LLM Call 记录。独立的任务状态和执行历史数据模型属于扩展阶段。
...
| `channel` | VARCHAR | 接入渠道；定时触发使用系统调度渠道 |
| `user_id` | VARCHAR | 用户或系统身份；定时触发按 `schedule.id` 区分 |
...
| **第四周** | 3 小时 | 多 Agent 演示 + 定时任务 + 工程化收尾 | 多 Agent 并存可用，定时任务到点自动触发，Session 跨重启恢复，项目主页可访问 |
...
- `notify` Tool 和通用 Webhook 通知渠道（域名白名单约束）
...
| **Provider/Eino 兼容性风险** | 各 LLM Provider 的 Function Calling、Stream、Token 计数、错误码细节不一致；MiniMax 通过 Eino-ext OpenAI connector 接入时还要正确保留多轮工具调用的完整响应；Eino-ext connector 仍在迭代 | 核心阶段先把 DeepSeek 和 MiniMax 跑稳，对 Function Calling、流式响应和多轮 Tool 消息做回归测试，其他 Provider 在扩展阶段做完整回归测试 |
| **Tool 执行安全风险** | 应用层白名单不是完整 Sandbox，可能影响 OryxOS 进程 | 严格限制内置 Tool 能力范围，核心阶段不建议在生产环境跑高敏感场景 |
| **Eino 版本稳定性风险** | Eino 仍为 0.x 版本（v0.9.x），API 可能在 1.0 前发生不兼容变动 | 锁定 Eino 特定版本，在 OryxOS 与 Eino 之间加薄适配层，升级前做完整回归 |
...
- 单节点快速部署文档（新手 30 分钟内完成部署）；生产部署与完整运维手册放在社区共建阶段
- 命令行工具有清晰的帮助和错误提示
...
| Demo | 验证能力 | 场景描述 | 验收标准 |
|------|---------|---------|---------|
| **Demo 一：每日天气** | 能力一+二（LLM + ReAct）、能力四（内置 HTTP Tool + `notify`）、定时任务（`AgentScheduler`） | 每天早上到点自动查天气、生成穿搭建议，通过 Webhook 推送到企业 IM 群 | 不需要人工触发，到点自动跑完整 ReAct 循环；调用一次 `http_get` 查天气、一次 `notify` 推送，两次出站都过 Sandbox 域名白名单且都写入 `tool_invocations`；`GET /api/v1/sessions/{id}` 能查到这次自动触发的完整对话记录 |
| **Demo 二：每日科技日报** | 能力四（Plugin Tool 方式一 SKILL.md 零代码 + 方式二 MCP）、能力三（Memory）、定时任务（`AgentScheduler`） | 每天到点自动汇总当日科技新闻并推送，且日报内容会体现用户之前说过的关注方向（比如"更关注 AI 和芯片"） | 业务方全程不写 Go 代码，只写 SKILL.md + `mcp_servers.yaml` + Profile 的 `schedules` 字段；LLM 自己决定调新闻 MCP 工具、自己组织日报、自己调推送 MCP 工具，OryxOS 不解析任务步骤；日报内容能体现 `MEMORY.md` 里记住的偏好 |

两个 Demo 都是"钟推"（`AgentScheduler` 到点自动触发），但都要能同时支持"人推"手动补跑一次做验证（`oryxos chat` 或 `POST /api/v1/agents/{name}/invoke`），验证同一个 Agent 不管从哪个入口触发，走的都是同一条 `AgentService` 链路。

---

## 14. 总结

OryxOS 是基于 Go 1.26 实现的面向企业场景的 Agent OS，装在企业自己的 K8s 或服务器上，作为统一底座跑各种业务 Agent，共享一套渠道接入、模型路由、工具调用、记忆系统、沙箱执行能力。

**OryxOS 的交付分两段：**

1. 核心阶段先用 Go 把 Agent OS 的运行时内核做扎实，这一层在能力上对齐业界开源 Agent OS 的基础层
...
- **Plugin 自定义工具 + 内置工具集**：内置文件/Shell/HTTP，业务方通过 SKILL.md + MCP 零代码扩展、MCP server 轻代码扩展、Go Tool 接口重代码扩展

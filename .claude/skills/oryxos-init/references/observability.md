...
The application logger has exactly two output modes, selected by `internal/app` from the validated
`config.ServerConfig.LogFormat`. Development defaults to a colored human-readable console sink;
production configuration selects the JSON sink. Both constructors wrap their sink in the required
redacting handler, so redaction and correlation behave identically in both modes:

```go
// NewLogger is the production sink: one JSON object per line.
...
// NewConsoleLogger is the development sink: colored, human-readable text lines.
func NewConsoleLogger(w io.Writer, level slog.Leveler) *slog.Logger {
	consoleHandler := NewConsoleHandler(w, &slog.HandlerOptions{Level: level})
	return slog.New(NewRedactingHandler(consoleHandler))
}
```

`NewConsoleHandler` is implemented on the standard library `log/slog` only: one human-readable line
per record with time, level, message, and `key=value` attributes, and the level label colored by
severity through ANSI escapes. It must not add a third-party logging dependency, and it is not a
machine-readable format; tests assert on the presence of the level name, message, and correlation
values, never on exact escape sequences.

The JSON sink writes one JSON object per line to its configured sink. Tests inject a buffer for
either mode. Use `logger.LogAttrs` or `logger.With` with typed `slog` attributes; do not build JSON
manually or format machine-readable fields into a message string. A record has the normal `slog`
fields plus the applicable correlation fields and event-specific attributes. Handlers, middleware,
and commands receive a plain `*slog.Logger` and stay mode-agnostic; only `internal/app` picks the
constructor.

Diagnostics and log events are emitted only through this logger. Production code under
`internal/**` must never call `fmt.Print*`, the built-in `print`/`println`, or write directly to
`os.Stdout` or `os.Stderr`; the pinned `forbidigo` linter enforces this. Deliberate user-facing
Cobra command output in `cmd/` is product output rather than logging and remains allowed.
...
`NewLogger` and `NewConsoleLogger` must each install it exactly as shown above. It is a `slog.Handler` wrapper, not a
...
Use table-driven tests with a buffer logger and the in-memory observer. JSON-mode rows parse each
log line into `map[string]any`; console-mode rows assert on complete human-readable text lines. Do
not make assertions by substring alone when the contract needs structural evidence.
...
| `console_format_and_correlation` | Build `NewConsoleLogger(...)` over a buffer; add all five correlation values; log one event | One human-readable line contains the level name, the event message, and each correlation value; the line is not a JSON object. |
| `console_redaction` | Repeat the `sanitized_error` and `direct_sensitive_attributes` setups through `NewConsoleLogger(...)` | Every captured console line contains neither secret; sensitive keyed values are `[REDACTED]` or omitted. |

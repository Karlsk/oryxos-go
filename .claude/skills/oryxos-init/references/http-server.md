...
contracts. The constructor receives a validated `config.ServerConfig`, an `observability.Observer`, and a
base `*slog.Logger` created through `observability.NewLogger` or `observability.NewConsoleLogger`
according to the configured log format.

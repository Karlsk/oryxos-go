...
server, select the log output mode, and bound shutdown. It does not load, represent, validate, or retain Profile business
...
| `log_format` | `console` | Exactly `console` or `json`, case-sensitive. `console` is the development colored-console sink; production deployments select `json`. |
...
config log_format: must be one of "console" or "json"
```
...
| --- | --- | --- |
| `defaults` | Empty YAML document | Exact defaults: `127.0.0.1:8080`, `log_format: console`, `5s`, `30s`, `5m`, `60s`, and `30s`. |
...
| `log_format_json` | `log_format: json` | Typed value is `LogFormatJSON`; every other field keeps its exact default. |
| `log_format_invalid` | Rows `log_format: xml` and `log_format: JSON` | Each error names `log_format` and permits only `console` or `json`; no case-insensitive match or fallback substitution occurs. |

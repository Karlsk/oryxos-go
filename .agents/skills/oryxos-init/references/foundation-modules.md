...
| Configuration | [configuration.md](configuration.md) | Service listen settings, HTTP timeouts, environment parsing, defaults, strict validation, and secret redaction; not Profile business configuration. |
| Structured logging and internal HTTP observations | [observability.md](observability.md) | `slog` JSON, correlation fields, internal metrics interface, and sensitive-data exclusion; no Prometheus endpoint. |

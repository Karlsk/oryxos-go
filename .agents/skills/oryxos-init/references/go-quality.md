...
- [Runtime library locking](#runtime-library-locking)
...
| --- | --- |
| `docs/DevelopmentGuide.md` | Local setup, supported Go version, `go tool` bootstrap, locked runtime library versions, Make targets, test/race/security workflow, and links to the three external style sources below. Never teach global tool installation. |
...
- Emit every diagnostic or log event through the application `slog` logger. Production code under
  `internal/**` must not call `fmt.Print*`, the built-in `print`/`println`, or write directly to
  `os.Stdout`/`os.Stderr`; the pinned `forbidigo` linter enforces this. Deliberate user-facing
...
## Runtime library locking

Framework and runtime libraries — Gin, Cobra, `gopkg.in/yaml.v3`, and later Eino core, Eino-ext
provider connectors, the official MCP Go SDK, GORM with `github.com/glebarez/sqlite`, and
`github.com/robfig/cron/v3` — are selected as the latest official stable release at
implementation time and locked as exact `go.mod` require versions before development starts: pick
the current stable version first, then lock it. `go.sum` and the `go mod tidy -diff` gate prove
the lock stays current, and upgrading a runtime library is a reviewed change with its own
rationale, never a floating or drive-by bump. Record each locked runtime version beside the tool
pins in `docs/DevelopmentGuide.md`.
...
`gocyclo`, `revive`, `errorlint`, `noctx`, `bodyclose`, and `forbidigo`. Configure `forbidigo`
to forbid `fmt.Print*`, the built-in `print`/`println`, and direct `os.Stdout`/`os.Stderr` writes
in non-test production code. Do not suppress generated or

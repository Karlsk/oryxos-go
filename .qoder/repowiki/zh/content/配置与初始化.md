...
## 更新摘要
**所做更改**
- 增强了配置系统的详细说明，包括严格的YAML验证、环境变量展开机制和超时配置
- 添加了完整的配置字段参考，涵盖所有支持的配置选项和默认值
- 更新了工作区结构说明以反映实际的目录和文件创建行为
- 增强了CLI命令参考，包含所有子命令的详细用法
- 添加了敏感信息保护和安全最佳实践说明
- 完善了初始化流程的架构图和执行步骤
...
|------|------|------|
| `AGENTS.md` | 项目指令 | 0600 |
| `SOUL.md` | Agent 人格 | 0600 |
| `USER.md` | 用户偏好 | 0600 |
| `memory/MEMORY.md` | 初始记忆文件 | 0600 |
| `mcp_servers.yaml` | MCP 服务器列表 | 0600 |
| `profiles/default.yaml` | 默认 Profile 模板 | 0600 |

### 初始化行为
- **安全创建**：仅创建不存在的目标；已存在的目标不会被覆盖
- **权限控制**：目录权限设置为 0750，文件权限设置为 0600，避免误写
- **符号链接防护**：拒绝符号链接指向的工作区目标，防止绕过安全检查
- **原子操作**：使用临时文件和原子链接确保文件创建的原子性
- **审计输出**：输出每个目标的创建结果（created/skipped），便于审计

### 状态检查
- **initialized**：当 `.oryxos` 及其全部强制目录与文件均存在时
- **not_initialized**：否则处于未初始化状态

```mermaid
flowchart TD
Start(["执行 init"]) --> EnsureRoot["确保 .oryxos 目录存在"]
EnsureRoot --> CreateDirs["创建 profiles/sessions/skills/logs/memory"]
CreateDirs --> CreateFiles["创建 AGENTS.md/SOUL.md/USER.md/memory/MEMORY.md/mcp_servers.yaml/profiles/default.yaml"]
CreateFiles --> CheckExists{"是否已存在?"}
